...
public class TransmissionSystem {
    ...
    public static void tick(Player player, ServerLevel level,
        ...
        // Skip all transmission if wearing full hazmat
        if (isFullHazmat(player)) return;
    ...
    private static void tickBlockExposure(Player player, ServerLevel level, BlockPos below,
        ...
        String lastType = player.getPersistentData().getString(NBT_BLOCK_STAND_TYPE);
        int ticks = player.getPersistentData().getInt(NBT_BLOCK_STAND_TICKS);
        ...
        if (ticks >= STAND_TICKS_REQUIRED) {
            ...
            for (MicrobeStrain strain : candidates) {
                // Only expose if strain has matching vector
                SpreadVector required = vectorForBlockType(blockType);
                if (strain.spreadVectors.contains(required)) {
                    // Lower chance than direct contact — environmental exposure is dilute
                    boolean blocked = !microbiome.tryInfect(strain, -0.4f, immune, registry);
                    // If blocked by hazmat... wait, we already returned early for full hazmat
                    // This branch only runs if not wearing full suit
                }
            }
        }
    }
    ...
    private static void tickAerosolExposure(Player player, ServerLevel level,
                                             HostMicrobiome microbiome, PlayerImmuneSystem immune,
                                             MicrobeRegistry registry) {
        // Only mobs within 6 blocks matter for aerosol
        level.getEntitiesOfClass(net.minecraft.world.entity.LivingEntity.class,
            player.getBoundingBox().inflate(6.0),
            e -> e != player && e.getPersistentData().contains("MM_PendingInfections"))
        .forEach(mob -> {
            net.minecraft.nbt.ListTag pending = mob.getPersistentData()
                .getList("MM_PendingInfections", net.minecraft.nbt.Tag.TAG_COMPOUND);
            for (int i = 0; i < pending.size(); i++) {
                long strainId = pending.getCompound(i).getLong("StrainId");
                ...
                if (strain.spreadVectors.contains(SpreadVector.AEROSOL) ||
                    ...
                    // Distance penalty — further away = less exposure
                    ...
                    // Spawn sneeze particles at the mob (visual feedback)
                    spawnAerosolParticles(level, mob);
                }
            }
        });
    }
    ...
    private static void tickContactExposure(Player player, ServerLevel level,
                                             HostMicrobiome microbiome, PlayerImmuneSystem immune,
                                             MicrobeRegistry registry) {
        // Contact from nearby mobs that have pending infections (2 block radius)
        level.getEntitiesOfClass(net.minecraft.world.entity.LivingEntity.class,
