...
public class TransmissionSystem {
    ...
    public static void tick(Player player, ServerLevel level,
        ...
        // Skip all transmission if wearing full hazmat — but suit absorbs strains externally
    ...
    private static void absorbEnvironmentalStrains(Player player, ServerLevel level,
        ...
        // Collect strains from infected mobs within 8 blocks via MobColony
        ...
        java.util.Random rng = new java.util.Random(
            level.getGameTime() ^ player.getUUID().getLeastSignificantBits());
        ...
        net.minecraft.world.entity.EquipmentSlot targetSlot = slots[rng.nextInt(slots.length)];
        net.minecraft.world.item.ItemStack piece = player.getItemBySlot(targetSlot);
        if (piece.getItem() instanceof com.grace.micromayhem.item.HazmatArmorItem) {
            // Respect 10s grace period after autoclaving
            long sterilizedAt = com.grace.micromayhem.util.NbtHelper.getTag(piece).getLong("SterilizedAt");
            if (sterilizedAt > 0 && System.currentTimeMillis() - sterilizedAt < 10_000L) return;
            com.grace.micromayhem.item.HazmatArmorItem.absorbStrain(piece, strain.strainId);
        }
    }
