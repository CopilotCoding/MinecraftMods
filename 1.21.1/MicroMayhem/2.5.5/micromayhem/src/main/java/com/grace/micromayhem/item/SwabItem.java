...
public class SwabItem extends Item {
    ...
    public InteractionResult useOn(UseOnContext ctx) {
        ...
        if (level instanceof ServerLevel serverLevel) {
            ...
            contaminateStack(contaminated, selectStrains(strains), "block");
            if (stack.getCount() == 1) {
                player.setItemInHand(ctx.getHand(), contaminated);
            } else {
                stack.shrink(1);
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (level instanceof ServerLevel serverLevel) {
            ...
            ItemStack contaminated = new ItemStack(this, 1);
            contaminateStack(contaminated, selectStrains(strains), sampleType);
            if (stack.getCount() == 1) {
                player.setItemInHand(hand, contaminated);
                return InteractionResultHolder.sidedSuccess(contaminated, level.isClientSide);
            } else {
                stack.shrink(1);
                if (!player.getInventory().add(contaminated)) {
                    player.drop(contaminated, false);
    ...
    /** Randomly select up to 3 strains from the list if there are more. */
    private static List<MicrobeStrain> selectStrains(List<MicrobeStrain> strains) {
        if (strains.size() <= 3) return strains;
        java.util.Collections.shuffle(strains);
        return strains.subList(0, 3);
    }
