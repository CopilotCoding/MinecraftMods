...
public class PlayerMicrobiome {
    ...
    public void loadFromTag(CompoundTag playerTag) {
        ...
        infections.clear();
        ListTag infList = tag.getList("Infections", Tag.TAG_COMPOUND);
        for (int i = 0; i < infList.size(); i++) {
            CompoundTag t = infList.getCompound(i);
            ActiveInfection inf = new ActiveInfection();
            inf.strainId = t.getLong("StrainId");
            inf.stage = InfectionStage.valueOf(t.getString("Stage"));
            inf.ticksRemaining = t.getInt("TicksRemaining");
            inf.latentTicksRemaining = t.getInt("LatentTicks");
            infections.put(inf.strainId, inf);
        }

        immuneStrains.clear();
        for (long id : tag.getLongArray("ImmuneStrains")) immuneStrains.add(id);

        bodyFlora.clear();
        for (long id : tag.getLongArray("BodyFlora")) bodyFlora.add(id);
    }
}
