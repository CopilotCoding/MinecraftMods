...
public class VaccineSyringeItem extends SyringeItem {
    ...
    public void appendHoverText(ItemStack stack, TooltipContext ctx, List<Component> lines, TooltipFlag flag) {
        lines.add(Component.literal("§a[VACCINE SYRINGE]").withStyle(ChatFormatting.BOLD));
        if (isLoaded(stack)) {
            long[] ids = getStrainIds(stack);
            lines.add(Component.literal("§7Provides immunity to §f" + ids.length + " strain(s)."));
        }
        lines.add(Component.literal("§7Right-click self or entity to administer."));
    }
}
