...
public class GeneticRecombination {
    ...
    public static List<MicrobeStrain> recombine(
        ...
        for (int c = 0; c < CANDIDATE_COUNT; c++) {
            ...
            // ---- v2.0 fields — must all be set or save() crashes ----
            child.aggressionFactor    = (parentA.aggressionFactor + parentB.aggressionFactor) / 2f
                + (rng.nextFloat() - 0.5f) * 0.1f;
            child.aggressionFactor    = Math.max(0f, Math.min(1f, child.aggressionFactor));
            child.defenseFactor       = (parentA.defenseFactor + parentB.defenseFactor) / 2f
                + (rng.nextFloat() - 0.5f) * 0.1f;
            child.defenseFactor       = Math.max(0f, Math.min(1f, child.defenseFactor));
            child.autoimmunePotential = rng.nextBoolean()
                ? parentA.autoimmunePotential : parentB.autoimmunePotential;
            if (child.autoimmunePotential == null) child.autoimmunePotential = AutoimmunePotential.NONE;
            child.lethalityProfile    = rng.nextBoolean()
                ? parentA.lethalityProfile : parentB.lethalityProfile;
            if (child.lethalityProfile == null) child.lethalityProfile = LethalityProfile.NON_LETHAL;
            child.primarySystem       = BodySystem.primaryAffinity(child);
            child.biomePreference     = rng.nextBoolean()
                ? parentA.biomePreference : parentB.biomePreference;
            if (child.biomePreference == null) child.biomePreference = BiomePressure.NEUTRAL;
            ...
            child.effectDurationTicks = (int)((parentA.effectDurationTicks + parentB.effectDurationTicks) / 2f);
