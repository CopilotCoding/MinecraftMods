...
public class MicrobeStrain {
    ...
    public static MicrobeStrain load(CompoundTag tag) {
        MicrobeStrain s = new MicrobeStrain();
        s.strainId   = tag.getLongOr("StrainId", 0L);
        s.genus      = tag.getStringOr("Genus", "");
        s.species    = tag.getStringOr("Species", "");
        s.type       = MicrobeType.valueOf(tag.getStringOr("Type", ""));
        s.effect     = MicrobeEffect.valueOf(tag.getStringOr("Effect", ""));
        ListTag vecs = tag.getListOrEmpty("Vectors");
        for (int i = 0; i < vecs.size(); i++)
            s.spreadVectors.add(SpreadVector.valueOf(vecs.getStringOr(i, "")));
        s.virulence           = tag.getIntOr("Virulence", 0);
        s.transmissibility    = tag.getFloatOr("Transmissibility", 0f);
        s.generation          = tag.getIntOr("Generation", 0);
        s.mutationAccumulator = tag.getFloatOr("MutationAccumulator", 0f);
        s.effectDurationTicks = tag.getIntOr("EffectDuration", 0);
        // v2.0
        s.aggressionFactor    = tag.getFloatOr("Aggression", 0f);
        s.defenseFactor       = tag.getFloatOr("Defense", 0f);
        s.autoimmunePotential = tag.contains("AutoimmunePotential")
            ? AutoimmunePotential.valueOf(tag.getStringOr("AutoimmunePotential", ""))
            : AutoimmunePotential.NONE;
        s.lethalityProfile    = tag.contains("LethalityProfile")
            ? LethalityProfile.valueOf(tag.getStringOr("LethalityProfile", ""))
            : LethalityProfile.NON_LETHAL;
        s.primarySystem       = tag.contains("PrimarySystem")
            ? BodySystem.valueOf(tag.getStringOr("PrimarySystem", ""))
            : BodySystem.DIGESTIVE;
        s.biomePreference     = tag.contains("BiomePreference")
            ? BiomePressure.valueOf(tag.getStringOr("BiomePreference", ""))
            : BiomePressure.NEUTRAL;
        s.relationshipSeed    = tag.getLongOr("RelationshipSeed", 0L);
        s.fullyCharacterised  = tag.getBooleanOr("FullyCharacterised", false);
        s.microscopeViewCount = tag.getIntOr("MicroscopeViews", 0);
        s.isWorldStrain       = tag.getBooleanOr("IsWorldStrain", false);
        s.hostCount           = tag.getIntOr("HostCount", 0);
        s.daysSinceLastHost   = tag.getIntOr("DaysSinceLastHost", 0);
        return s;
    }
